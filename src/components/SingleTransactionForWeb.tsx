...
const SingleTransactionForWeb: FC = () => {
  const params = useParams();
  let singleTransaction;

  const {
    data: customerInvoiceData,
    isLoading: isCustomerInvoiceDataLoading,
    isSuccess: isCustomerInvoiceDataSuccesss,
  } = useQuery({
    queryKey: ["buyer-invoice-data"],
    queryFn: async () => {
      const transactions = await VehicleTransactionService.getAll();
      return transactions;
    },
  });
  ...
  if (isCustomerInvoiceDataLoading) {
    console.log("loading ....");
  }

  if (isCustomerInvoiceDataSuccesss || isSellerInvoiceDataSuccesss) {
    console.log(customerInvoiceData);
    console.log(params.id);

    if (params.id?.substring(0, 3) === "620") {
      singleTransaction = customerInvoiceData?.transaction?.find(
        (tr: any) => tr.code === params.id
      );
    } else if (params.id?.substring(0, 3) === "630") {
  ...
  return (
    <section className="w-full min-h-screen overflow-y-hidden bg-primaryBg">
      <h1 className="flex gap-10 p-3 m-3 text-2xl text-center text-white bg-formHeaderBg font-roboto-bold">
        <NavLink
          to={`/intro/${
            params.id?.substring(0, 3) === "620" ? "buyer" : "seller"
          }-invoice`}
          className="mt-1"
        >
          <RiArrowGoBackLine />
      ...
      <div className="flex items-center justify-center w-full mt-10 ">
        <div className="relative w-3/4 p-3 shadow-md h-[500px] bg-formBg">
          ...
          {isCustomerInvoiceDataSuccesss && singleTransaction && (
            <div>
              <div className="mx-10">
                      ...
                      <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td className="px-6 py-4">
                          {params.id?.substring(0, 3) === "620"
                            ...
                            : singleTransaction.sellingPrice}
                        </td>
                      </tr>
                    </tbody>
                  </table>
...
export default SingleTransactionForWeb;
