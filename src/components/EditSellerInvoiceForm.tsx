...
export async function action({ request }: any) {
  ...
  //seller edit mode
  const sellerCode = requestData.get("edit");
  ...
  try {
    await VehicleSellingTransactionService.edit(values, sellerCode);
...
const EditSellerInvoiceForm: FC = () => {
  ...
  if (isSellerInvoiceDataSuccesss) {
    console.log(
      params.id,
      sellerInvoiceData?.transaction?.find((tr: any) => tr.code === params.id)
    );
  }
  ...
  return (
    <>
      ...
      <Form className="w-full p-4 mt-20 bg-formBg" method="put" replace>
        <input type="hidden" name="edit" value={params.id} />
          ...
          {buyingTransactionData.length !== 0 && (
            <div>
              ...
              <input
                ...
                name="model"
                value={
                  buyingTransactionData.reverse().at(0)?.customer.secondName
                }
                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
                required
              />
...
export default EditSellerInvoiceForm;
