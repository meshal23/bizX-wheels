...
const Invoice = () => {
  ...
  const { data: buyerInvoiceData, isLoading: isBuyerInvoiceDataLoading } =
    useQuery({
  ...
  if (isBuyerInvoiceDataLoading) {
    console.log("loading transactions");
  }

  console.log(buyerInvoiceData);
  ...
  return (
    <section className="relative grid w-full min-h-screen p-5 overflow-y-scroll scrollbar-thin scrollbar-webkit place-content-center bg-primaryBg">
      ...
      <div className="absolute w-11/12 overflow-x-auto transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        ...
        <div
          className={`overflow-y-scroll scrollbar-thin h-80 scrollbar-webkit top-24`}
        >
          {printView && <PrintView transaction={printViewData} />}
          {isBuyerInvoiceDataLoading ? (
            ...
            <table className="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400">
              ...
              <tbody>
                {buyerInvoiceData?.transaction
                  // .filter((type: any) => type.customerType === "buyer")
                  .map((transaction: any) => {
                    return (
                      <tr
                      ...
                      >
                        <th
                          scope="row"
                          className="px-6 py-4 mt-auto font-medium text-gray-900 whitespace-nowrap dark:text-white"
                        >
                          <input
                            type="checkbox"
                        ...
                        <td className="px-6 py-4">
                          {transaction?.buyingTransactionDate}
                        </td>
                        <td className="px-6 py-4">
