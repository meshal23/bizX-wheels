...
const Customer: FC = () => {
  ...
  if (isError) {
    return toast.error("Failed to get the buyers");
  }
  ...
  return (
    <section className="relative grid w-full min-h-screen p-5 overflow-y-scroll scrollbar-thin scrollbar-webkit place-content-center bg-primaryBg">
      ...
      <div className="absolute w-3/4 transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        {/* <button
          type="button"
        ...
        >
          <FaUserPlus size={20} style={{ marginRight: 8, marginBottom: 2 }} />
          Add Customer
        </button> */}
        <NavLink
          to="/intro/create-buyer"
          className="text-white bg-formHeaderBg hover:shadow-lg focus:ring-4 focus:outline-none focus:ring-[#3b5998]/50 font-medium rounded-lg text-sm px-3 py-2.5 text-center inline-flex items-center dark:focus:ring-[#3b5998]/55 me-2 mb-2"
        >
          <FaUserPlus size={20} style={{ marginRight: 8, marginBottom: 2 }} />
          Add Customer
        ...
        <div className="overflow-y-scroll scrollbar-thin scrollbar-webkit top-24 h-80">
          {isLoading ? (
            ...
            <table className="w-full text-sm text-left text-gray-500 h-52 rtl:text-right dark:text-gray-400">
              ...
              <tbody className="">
                {data.customer
                  // .filter((cusVer: any) => cusVer.website == "buyer")
                  .map((cus: any) => {
                    return (
                      <tr
                        key={cus.code}
                      ...
                      >
                        ...
                        <td className="px-6 py-4">{cus.nicNumber}</td>
                        <td className="px-6 py-4">{cus.phone1}</td>
                        <td className="px-6 py-4">{cus.notes}</td>
                      </tr>
                    );
                  })}
